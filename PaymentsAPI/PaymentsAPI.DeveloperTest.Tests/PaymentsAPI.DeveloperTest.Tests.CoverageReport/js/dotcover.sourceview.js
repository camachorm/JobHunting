...
function highlightRanges(rawRanges) {
    ...
    var maxEscapeSequenceLength =
        getMaxStringLength(
            escapeSequenceRegularExpression.source.match(new RegExp(escapeSequenceRegularExpression.source.substr(1),
                "g")));
    ...
    while (position < source.length) {
        ...
        if (line == currentRange.line && column == currentRange.column) {
            if (previousCoverageStatus == null) {
                ...
                result = result.concat(source.substring(previousPosition, position),
                    "<a class='",
                    (currentRange.covered ? "covered" : "uncovered"),
                    "' id='s",
                    currentRange.line,
                    ".",
                    currentRange.column,
        ...
        // testing for the end of the range, but taking into account possible wrong range that goes beyound end of line
            ...
            if (previousCoverageStatus != null) {
                ...
                while (currentRange != null &&
                    (line > currentRange.endLine || line == currentRange.endLine && column >= currentRange.endColumn));
